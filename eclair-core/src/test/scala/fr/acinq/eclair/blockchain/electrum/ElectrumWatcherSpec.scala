...
import fr.acinq.eclair.blockchain.{GetTx, GetTxResponse, WatchConfirmed, WatchEventConfirmed, WatchEventSpent, WatchSpent}
...
class ElectrumWatcherSpec extends TestKit(ActorSystem("test")) with FunSuiteLike with BitcoindService with ElectrumxService  with BeforeAndAfterAll with Logging {
  ...
  test("get transaction") {
...

    {
      ...
      probe.send(watcher, GetTx(txid))
      val res = probe.expectMsgType[GetTxResponse]
