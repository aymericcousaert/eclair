...
import fr.acinq.eclair.blockchain.{GetTxResponse, UtxoStatus, ValidateResult}
...
class ExtendedBitcoinClient(val rpcClient: BitcoinJsonRPCClient) {
  ...
  def getTransactionMeta(txId: String)(implicit ec: ExecutionContext): Future[GetTxResponse] =
    for {
    ...
    } yield GetTxResponse(txid = ByteVector32.fromValidHex(txId), tx_opt, timestamp.toLong)
